...
from awsiot.greengrasscoreipc.model import ResourceNotFoundError, InvalidArgumentsError, ServiceError, UnauthorizedError, ConflictError
...
try:
    log_file_path = IPC_CLIENT.get_configuration( key_path=[LOG_FILE_PATH_CONFIG]).value[LOG_FILE_PATH_CONFIG]
    log_level = (IPC_CLIENT.get_configuration( key_path=[LOG_LEVEL_CONFIG]).value[LOG_LEVEL_CONFIG]).upper()
...
ROBOT_ENABLED = IPC_CLIENT.get_configuration(key_path=[ROBOT_ENABLED_CONFIG]).value[ROBOT_ENABLED_CONFIG]
...
ROBOT_CONTROL_MOVE_RESPONSE_STATUS_VALUES = {
    'OK': 200,
    'ERROR': 500
}
...
def subscribe_to_robot_move_responses():
    robot_control_move_response_topic = IPC_CLIENT.get_configuration( key_path=[ROBOT_CONTROL_MOVE_RESPONSE_TOPIC_CONFIG]).value[ROBOT_CONTROL_MOVE_RESPONSE_TOPIC_CONFIG]
    ...
    logger.info(f"Successfully subscribed using aws.greengrass.ipc.pubsub to topic {robot_control_move_response_topic}")
        ...
        if not ROBOT_CONTROL_MOVE_RESPONSE_EVENT.is_set():
            ...
            if got_message:
                ...
                if "status" in message:
                    if message["status"] == ROBOT_CONTROL_MOVE_RESPONSE_STATUS_VALUES['OK']:
                        ...
                        if LAST_VALIDATED_MOVE == message['lan']:
                            ...
                            ROBOT_CONTROL_MOVE_RESPONSE_EVENT.clear()
                            logger.critical("Robot Control Move Response verification success")
                        else:
                            raise RoboticArmException(f"Last validated move: {LAST_VALIDATED_MOVE} different than Current processed move: {message['lan']}")
                    elif message["status"] == ROBOT_CONTROL_MOVE_RESPONSE_STATUS_VALUES['ERROR']:
                    ...
                    ROBOT_CONTROL_MOVE_RESPONSE_REASON = "Invalid robot control move response message - Status missing."
                    raise RoboticArmException(ROBOT_CONTROL_MOVE_RESPONSE_REASON)
            else:
...
def publish_robot_control_move_request(game_request):
    ...
    try:
        ...
        robot_control_move_topic = IPC_CLIENT.get_configuration(key_path=[ROBOT_CONTROL_MOVE_TOPIC_CONFIG]).value[ROBOT_CONTROL_MOVE_TOPIC_CONFIG]
        if robot_control_move_topic:
            logger.debug(f"Game Request: {game_request}")
            publish_message(game_request, robot_control_move_topic)
        else:
            raise RoboticArmException("Robot Control Move Request topic not specified in topic config {ROBOT_CONTROL_MOVE_TOPIC_CONFIG}")
    except Exception:
        logger.exception("Unable to publish robot control move request")
        ROBOT_CONTROL_MOVE_RESPONSE_EVENT.clear()

def subscribe_to_board_validated_moves():
    board_validated_topic = IPC_CLIENT.get_configuration( key_path=[BOARD_VALIDATIONS_RESPONSE_TOPIC_CONFIG]).value[BOARD_VALIDATIONS_RESPONSE_TOPIC_CONFIG]
    ...
    logger.info(f"Successfully subscribed using aws.greengrass.ipc.pubsub to topic {board_validated_topic}")

def subscribe_to_board_processed_moves():
    board_processed_topic = IPC_CLIENT.get_configuration( key_path=[BOARD_PROCESSED_TOPIC_CONFIG]).value[BOARD_PROCESSED_TOPIC_CONFIG]
    ...
    logger.info(f"Successfully subscribed using aws.greengrass.ipc.pubsub to topic {board_processed_topic}")

def subscribe_to_board_started_moves():
    board_started_topic = IPC_CLIENT.get_configuration( key_path=[BOARD_START_RESPONSE_TOPIC_CONFIG]).value[BOARD_START_RESPONSE_TOPIC_CONFIG]
    logger.info(f"Subscribing to topic {board_started_topic}")
    IPC_CLIENT.subscribe_to_topic(
            topic=board_started_topic,
            on_stream_event=on_board_started_move,
    )
    logger.info(f"Successfully subscribed using aws.greengrass.ipc.pubsub to topic {board_started_topic}")
...
def on_board_validated_move(event):
    ...
    try:
        if not BOARD_VALIDATION_EVENT.is_set():
            logger.warning("Board validation game event wasn't set. Ignoring message as possible duplicate.")
            ...
            if got_message:
                ...
                if "status" in message:
                    if message["status"] == BOARD_STATUS_VALUES['OK']:
                        ...
                        raise SmartBoardException(f"Invalid move detected by Board  {'- ' + message['reason'] if 'reason' in message else None}")
                    else:
                        BOARD_VALIDATED_REASON = "Invalid game board validation message - Status invalid."
                        raise SmartBoardException("Invalid game board validation message - Status invalid.")
                else:
                    BOARD_VALIDATED_REASON = "Invalid game board validation message - Status missing."
                    raise SmartBoardException("Invalid game board validation message - Status missing.")
            else:
                BOARD_VALIDATED_REASON = "Invalid game board validation message - No Message or Invalid format"
                raise SmartBoardException("Invalid game board validation message - No Message or Invalid format")
...
def on_board_processed_move(event):
    ...
    try:
        if not BOARD_PROCESSED_EVENT.is_set():
            logger.warning("Board processed game event wasn't set. Ignoring message as possible duplicate.")
            ...
            if got_message:
                ...
                if "status" in message:
                    if message["status"] == BOARD_STATUS_VALUES['OK']:
                        ...
                        raise SmartBoardException("Invalid game board processed message - Status invalid.")
                else:
                    BOARD_PROCESSED_REASON = "Invalid game board processed message - Status missing."
                    raise SmartBoardException("Invalid game board processed message - Status missing.")
            else:
                BOARD_PROCESSED_REASON = "Invalid game board processed move message - No Message or Invalid format"
                raise SmartBoardException("Invalid game board processed message - No Message or Invalid format")
...
def on_board_started_move(event):
    ...
    try:
        if not BOARD_START_EVENT.is_set():
            logger.warning("Board started game event wasn't set. Ignoring message as possible duplicate.")
            ...
            if got_message:
                ...
                if "taskToken" in message:
                    if LAST_TASK_TOKEN is None:
                        ...
                        logger.info("Current task token is different, updating last known task token")
                        LAST_TASK_TOKEN = message["taskToken"]
                    elif LAST_TASK_TOKEN == message["taskToken"]:
                        raise SmartBoardException(f"Last task token is the same as the current token, potential duplicated message. {message['taskToken']}")
                    else:
                ...
                if "status" in message:
                    if message["status"] == BOARD_STATUS_VALUES['OK']:
                        ...
                        raise SmartBoardException(f"Invalid start position detected by Board  {'- ' + message['reason'] if 'reason' in message else None}")
                    else:
                        BOARD_STARTED_REASON = "Invalid start position message- Status invalid."
                        raise SmartBoardException("Invalid start position message - Status invalid.")
                else:
                    BOARD_STARTED_REASON = "Invalid start position message - Status missing."
                    raise SmartBoardException("Invalid start position message - Status missing.")
            else:
                BOARD_STARTED_REASON = "Invalid start position message - No Message or Invalid format"
                raise SmartBoardException("Invalid start position message - No Message or Invalid format")
...
def publish_board_validation_request(game_request):
    ...
    try:
        BOARD_VALIDATION_EVENT.set()
        logger.info(f"Publishing Board Validation Request - Event set {BOARD_VALIDATION_EVENT.is_set()}")
        board_validations_topic = IPC_CLIENT.get_configuration(key_path=[BOARD_VALIDATIONS_TOPIC_CONFIG]).value[BOARD_VALIDATIONS_TOPIC_CONFIG]
        if board_validations_topic:
            logger.debug(f"Game Request: {game_request}")
            publish_message(game_request, board_validations_topic)
        else:
            raise SmartBoardException("Board validations topic not specified in topic config {BOARD_VALIDATIONS_TOPIC_CONFIG}")
    except Exception:
...
def publish_board_start_request(game_request):
    ...
    try:
        ...
        board_start_topic = IPC_CLIENT.get_configuration(key_path=[BOARD_START_TOPIC_CONFIG]).value[BOARD_START_TOPIC_CONFIG]
        if board_start_topic:
            logger.debug(f"Game Request: {game_request}")
            publish_message(game_request, board_start_topic)
        else:
            raise SmartBoardException("Board start topic not specified in topic config {BOARD_START_TOPIC_CONFIG}")
    except Exception:
...
def get_core_device_named_shadow():
    ...
    try:
        core_named_shadow = IPC_CLIENT.get_configuration( key_path=[CORE_MAIN_NAMED_SHADOW_CONFIG]).value[CORE_MAIN_NAMED_SHADOW_CONFIG]
        logger.info(f"Getting shadow states for named shadow {core_named_shadow} of thing {core_named_shadow}")
        response = IPC_CLIENT.get_thing_shadow(thing_name=core_named_shadow, shadow_name=core_named_shadow)
        shadow_doc = json.loads(str(response.payload, 'utf-8'))
        if shadow_doc:
            desired = shadow_doc['state']['desired'] if 'desired' in shadow_doc['state'] else None
            reported = shadow_doc['state']['reported'] if 'reported' in shadow_doc['state'] else None
            delta = shadow_doc['state']['delta'] if 'delta' in shadow_doc['state'] else None
            
            logger.debug(f"Shadow Document: {shadow_doc}")
            
        return desired, reported, delta
    except InvalidArgumentsError:
        logger.error(f"Cannot get shadow states for named shadow {core_named_shadow} of thing {core_named_shadow}")
        logger.exception(INVALID_ARGUMENTS_ERR_MSG)
    except ResourceNotFoundError:
        logger.error(f"Cannot get shadow states for named shadow {core_named_shadow} of thing {core_named_shadow}")
        logger.exception("ResourceNotFoundError - The requested local shadow document can't be found.")
    except ServiceError:
        logger.error(f"Cannot get shadow states for named shadow {core_named_shadow} of thing {core_named_shadow}")
...
def update_core_device_named_shadow(desired=None, reported=None):
    ...
    try:
        core_named_shadow = IPC_CLIENT.get_configuration(key_path=[CORE_MAIN_NAMED_SHADOW_CONFIG]).value[CORE_MAIN_NAMED_SHADOW_CONFIG]
        logger.info(f"Updating shadow states for named shadow {core_named_shadow} of thing {core_named_shadow}")   
        ...
        if desired and reported:
            ...
            raise CoreDeviceException("Cannot update core device's named shadow with desired and reported states - both desired and reported states cannot be empty")
        
        payload_json = json.dumps(payload_json_dict, ensure_ascii=False)
        payload = bytes(payload_json, "utf-8")
        # logger.debug(f"Payload: {payload}")
        response = IPC_CLIENT.update_thing_shadow(thing_name=core_named_shadow, shadow_name=core_named_shadow, payload=payload)
        shadow_doc = json.loads(str(response.payload, 'utf-8'))
        logger.debug(f"Shadow Document: {shadow_doc}")
    
    except ConflictError:
        logger.error(f"Cannot update shadow states for named shadow {core_named_shadow} of thing {core_named_shadow}")
        logger.exception("ConflictError - The local shadow service encountered a version conflict during the update operation. This occurs when the version in the request payload doesn't match the version in the latest available local shadow document.")
    except InvalidArgumentsError:
        logger.error(f"Cannot update shadow states for named shadow {core_named_shadow} of thing {core_named_shadow}")
        logger.exception(INVALID_ARGUMENTS_ERR_MSG)
    except ServiceError:
...
def publish_message(message, topic):
    try:
        logger.info(f"Sending message to topic {topic} using aws.greengrass.ipc.mqttproxy.")
        ...
        logger.warning("Cannot send message to topic using aws.greengrass.ipc.mqttproxy.")
        try:
            logger.info( f"Sending message to topic {topic} using aws.greengrass.ipc.pubsub." )
...
def publish_game_response(task_token, status, reason=None):
    try:
        ...
        game_responses_topic = IPC_CLIENT.get_configuration(key_path=[GAME_RESPONSES_TOPIC_CONFIG]).value[GAME_RESPONSES_TOPIC_CONFIG]
        if game_responses_topic:
            if status and reason and task_token:
                ...
                raise GameException("Cannot publish game response - status and reason and taskToken cannot be empty")
        
            logger.debug(f"Game Response: {message}")
            publish_message(message, game_responses_topic)
        else:
            raise GameException("Game response topic not specified in topic config {GAME_RESPONSES_TOPIC_CONFIG}")
    except Exception:
        logger.exception("Unable to publish game response")

def subscribe_to_game_requests():
    # Cloud requestor topic
    game_requests_topic = IPC_CLIENT.get_configuration( key_path=[GAME_REQUESTS_TOPIC_CONFIG]).value[GAME_REQUESTS_TOPIC_CONFIG]
    ...
    logger.info( f"Successfully subscribed using aws.greengrass.ipc.mqttproxy to topic {game_requests_topic}" )
...
def on_game_request(event):
    ...
    try:
        ...
        if got_message:
            ...
            # Move action
            if "action" in message and (message["action"]).lower() in (GAME_REQUEST_ACTIONS.values()):
                if "taskToken" in message and "sessionId" in message:
                    # Check if Game manager is busy
                    if BUSY_EVENT.is_set():
                        logger.error(f"Game Manager BUSY! - Cannot process Game Request - {message['sessionId']}" )
                        thread = Thread(target=publish_game_response, args=(
                        ...
                        # Start action
                        if "action" in message and (message["action"]).lower() == (GAME_REQUEST_ACTIONS['START']).lower():
                            ...
                            if LAST_GAME_ID == CURRENT_GAME_ID:
                                raise GameException(f"Invalid game request, sessionId {CURRENT_GAME_ID} matches last ended game {LAST_GAME_ID}")
                        
                        # Move action
                        if "action" in message and (message["action"]).lower() == (GAME_REQUEST_ACTIONS['MOVE']).lower():
                            if GAME_IN_PROGRESS and message['sessionId'] != CURRENT_GAME_ID:
                                raise GameException(f"Invalid game request, sessionId {message['sessionId']} does not match current in-progress game {CURRENT_GAME_ID}")
                            if not GAME_IN_PROGRESS:
                                raise GameException("Invalid game request - Game is not in progress")
                    ...
                    raise GameException("Invalid game request - sessionId and/or taskToken not present")
            else:
                raise GameException("Invalid game request - action not present or invalid")
        else:
        ...
        logger.error(f"Error message sent - {message['taskToken'] if 'taskToken' in message else 'No Task Token Given'}")
...
def board_validate(game_request):
    ...
    logger.info(f"Validating move request with board - Attempt {board_validate_retry_num} - Event Set {BOARD_VALIDATION_EVENT.is_set()}")
...
def board_verify_move():
    ...
    if not BOARD_PROCESSED_OK:
        BOARD_PROCESSED_EVENT.set()
        elapsed_time = 0
        logger.info(f"Verifying board processed move - Attempt {board_verify_retry_num} - Event Set {BOARD_PROCESSED_EVENT.is_set()}")
...
def board_start(game_request):
    ...
    logger.info(f"Validating start request with board - Attempt {board_start_retry_num} - Event Set {BOARD_START_EVENT.is_set()}")
...
def robot_move(game_request):
    ...
    logger.info(f"Instructing Robotic Arm to make a move - Attempt {robot_move_retry_num} - Event Set {ROBOT_CONTROL_MOVE_RESPONSE_EVENT.is_set()}")
...
def process_game_request(game_request):
    ...
    logger.critical(f"Processing game request - sessionId {game_request['sessionId']} - Full Move {fullmove_num} - Action {game_request['action']}")
    logger.debug(f"Game request {game_request}")
    ...
    if (game_request['action']).lower() == str(GAME_REQUEST_ACTIONS['START']).lower():
        ...
        try:
            ...
            logger.error("Board start game request failed - Can't start game even after retrying.")
            BOARD_STARTED_REASON = "Board start game request failed - Can't start game even after retrying." if BOARD_STARTED_REASON is None else BOARD_STARTED_REASON
    ...
    # * Board action 'move'
        ...
        try:
            ...
            logger.error("Board validation failed - Cant validate move even after retrying.")
            BOARD_VALIDATED_REASON = "Board validation failed - Cant validate move even after retrying." if BOARD_VALIDATED_REASON is None else BOARD_VALIDATED_REASON
        ...
        if BOARD_VALIDATED_OK:
            ...
            if ROBOT_ENABLED:
                ...
                try:
                    ...
                    ROBOT_CONTROL_MOVE_RESPONSE_REASON = "Robot Control Move Request failed - Cannot confirm move has been executed even after retrying." if ROBOT_CONTROL_MOVE_RESPONSE_REASON is None else ROBOT_CONTROL_MOVE_RESPONSE_REASON
                ...
                if ROBOT_CONTROL_MOVE_RESPONSE_OK:
                    ...
                    try:
                        ...
                        BOARD_PROCESSED_REASON = BOARD_MOVE_VERIFICATION_FAILED_MSG if BOARD_PROCESSED_REASON is None else BOARD_PROCESSED_REASON
                    ...
                    if BOARD_PROCESSED_OK:
                        ...
                        halfmove_clock = int((game_request["after"].split('/')[7]).split(' ')[4])
                        is_checkmate = True if "#" in game_request["san"] else False
                        if halfmove_clock == 100 or is_checkmate:
                            logger.critical(f"Game is in final state - {game_request['action']}")
                            GAME_IN_PROGRESS = False
                            GAME_ENDED = True
                ...
                logger.warning("Robot Control is disabled - Processing Move without Robotic Arms")
...
def main():
    try:
        ...
        if ROBOT_ENABLED:
            ...
            logger.warning("Robot Control is DISABLED - Not Subscribed to Robot Control Topics")
        ...
        try:
            while True:
                ...
                if game_request is None:
                    ...
                    logger.critical(f"Handling Game request for action {game_request['action']} - sessionId {game_request['sessionId']}")
                    
                    # Highlight new game request on log
                    if game_request['action'] == GAME_REQUEST_ACTIONS['START']:
                        logger.warning(f"New Game request - sessionId {game_request['sessionId']}")
                    ...
                    update_core_device_named_shadow(desired={
                    ...
                    if delta:
                        ...
                        logger.critical(f"Processed Game request with status: {status} {'- reason ' + reason if reason != None else ''} ")
                        ...
                        if status == 200:
                            logger.info(f"Sending Game response message with status {status}")
                        else:
                            logger.warning(f"Sending Game response message with status {status} {'- reason ' + reason if reason != None else None} ")
                        ...
                        status = 400
                        logger.warning(f"Sending Game response message with status {status} - reason {delta_error_msg}")
                        publish_game_response(game_request['taskToken'], status, delta_error_msg)
                    ...
                    logger.critical(f"Game request processed - sessionId {game_request['sessionId']}")
