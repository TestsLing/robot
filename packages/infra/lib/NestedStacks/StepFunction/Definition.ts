...
export class Definition extends Construct {
  ...
  constructor(
    ...
    const catchError = new NodejsFunction(self, "Catch Error Func", {
      entry: path.join(__dirname, "lambdas", "catchError", "index.mjs"),
    ...
    if (self.node.tryGetContext("iotDevice") === "true") {
      ...
      const startIoTSession = new NodejsFunction(self, "Initialise IoT", {
        entry: path.join(__dirname, "initialiseIoT", "index.mjs"),
        bundling: {
      ...
      const sendMoveToRobotArm = new NodejsFunction(
        ...
        "Send Move to Device",
        {
          entry: path.join(__dirname, "toRobotArm", "index.mjs"),
          bundling: {
            externalModules: [
    ...
    const isGameOver = new NodejsFunction(self, "Game Over Function", {
      entry: path.join(__dirname, "lambdas", "gameOver", "index.mjs"),
      environment: { GRAPHQL_URL: graphqlApi.graphqlUrl },
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: {
        externalModules: ["@aws-sdk/*"],
    ...
    const bedrockMove = new python.PythonFunction(this, "Bedrock Move", {
      ...
      layers: [
        ...
        lambda.LayerVersion.fromLayerVersionArn(
          ...
          `arn:aws:lambda:${stack.region}:017000801446:layer:AWSLambdaPowertoolsPythonV3-python311-x86_64:2`
        ),
      ],
      initialPolicy: [
        new iam.PolicyStatement({
    ...
    const importModelsMove = new lambda.Function(self, "Imported Models Move", {
      code: lambda.Code.fromAsset(
        path.join(__dirname, "lambdas", "moveFunctions", "importedModels")
      ),
      runtime: lambda.Runtime.PYTHON_3_11,
